...
class grid:
    ...
    def set_text(self,x,y,char):
        self.canvas.text(content=char, pos=[x,y], font_size=20, color=0xFFFFFF)
#智能体
class agent:
...
for i in range(grid_width):
    for j in range(grid_height):
        grid_world.set_text(i/grid_width,j/grid_height,state_list[i][j])
...
for i in range(grid_width):
    for j in range(grid_height):
       gui.set_text(i/grid_width,j/grid_height,state_list[i][j])
while gui.running:
    N+=1
    if(N==dt*60):
        N=0
