...
for main_e in range(main_epoch):
    ...
    for i in range(dataset_size):
        ...
        for k in range(5):
            result[k]=main_network(torch.tensor([x,y,k],dtype=torch.float32))
    ...
    for i in range(grid_width):
        for j in range(grid_height):
            result=torch.zeros(5)
            for k in range(5):
                tmp=main_network(torch.tensor([i,j,k],dtype=torch.float32))
                # print(tmp)
                result[k]=tmp
...
for i in range(grid_width):
    for j in range(grid_height):
        result=torch.zeros(5)
        for k in range(5):
            result[k]=main_network(torch.tensor([i,j,k],dtype=torch.float32))
        policy=torch.argmax(result)
        print(f"{i} {j} {result} {policy}")
