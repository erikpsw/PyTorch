...
class Network(nn.Module):
    def __init__(self):
        ...
        # self.mlp_s1=nn.Linear(3,300)
        # self.mlp_s2=nn.Linear(300,1)
        
        self.mlp1=nn.Linear(7,40)
...
epochs=10
mse = nn.MSELoss()
optimizer = optim.Adam(main_network.parameters(), lr=0.0001, betas=(0.9, 0.98), eps=1e-9)
...
for main_e in range(main_epoch):
    epsilon=1-main_e/main_epoch
    for epoch in range(epochs):
        batch=random.sample(dataset,batch_size)
        input=torch.zeros((batch_size,7))
        
        R=torch.zeros(batch_size)
        next_input=torch.zeros((5,batch_size,7))
        for i,data in enumerate(batch):
            input[i,0]=data[0][0] # x
            input[i,1]=data[0][1] # y
            input[i,2+data[1]]= 1. # q
            for j in range(5):
                next_input[j,i,0]=data[3][0]
                next_input[j,i,1]=data[3][1] 
                next_input[j,i,2+j]=1.
            R[i]=data[2]
            
        # print(R)
        # print(input)
        # print(next_input)
        result=torch.zeros(5,batch_size)
        for j in range(5):
            result[j,:]=target_network(next_input[j,:]).reshape(batch_size)
        # print(result)
        max_q=torch.zeros(batch_size)
        for t in range(batch_size):
            max_q[t]=torch.max(result[:,t])
        y_T=R+gamma*max_q
        # print(y_T)
        for i in range(learning_epoch):
            optimizer.zero_grad()
            main_output=main_network(input).reshape(batch_size)
    ...
    for i in range(dataset_size):
        ...
        for k in range(5):
            input=torch.tensor([x,y,0,0,0,0,0],dtype=torch.float32)
            input[2+k]=1
            result[k]=main_network(input)
        cur_dir=torch.argmax(result)
    ...
    for i in range(grid_width):
        for j in range(grid_height):
            result=torch.zeros(5)
            for k in range(5):
                input=torch.tensor([i,j,0,0,0,0,0],dtype=torch.float32)
                input[2+k]=1
                tmp=main_network(torch.tensor(input,dtype=torch.float32))
                # print(tmp)
                result[k]=tmp
            policy=torch.argmax(result)
            print(f"{i} {j} {result} {policy}")
    print("________________________________________")
...
for i in range(grid_width):
    for j in range(grid_height):
        ...
        for k in range(5):
            input=torch.tensor([i,j,0,0,0,0,0],dtype=torch.float32)
            input[2+k]=1
            result[k]=main_network(torch.tensor(input,dtype=torch.float32))
        policy=torch.argmax(result)
        print(f"{i} {j} {result} {policy}")
